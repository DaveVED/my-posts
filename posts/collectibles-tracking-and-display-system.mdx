...
There were a few key arcitecture questions I needed to answer before getting started.
...
For whatever reason i've decdied to go with option two and have it as a subdomain. I might turn this into a shopping card some day, which was the main driver here. I'd like to keep the logic isolated.

### Where should lthis live (github)

My personal site [davedennis.dev](https://davedennis.dev) is setup with [turbo repo](https://turbo.build/) [*monorepo solution*](https://turbo.build/repo/docs#the-monorepo-solution) and I love it. If it was a net new domain i'd create a new repo for this, but since it's a subdomain i'm going to just add a new `app` to my monorepo. It'll look something like this.
...
It's very straight forward, and it going to let me grow my  [davedennis.dev](https://davedennis.dev) domain as needed.

> They define the setup like such "We recommend starting with splitting your packages into apps/ for applications and services and packages/ for everything else, like libraries and tooling.". See [here](https://turbo.build/repo/docs/crafting-your-repository/structuring-a-repository#specifying-packages-in-a-monorepo) for more details.