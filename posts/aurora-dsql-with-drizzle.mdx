...
We first need to get started by creating a basic setup for our project that may look something like this.
```bash
.
├── bun.lockb
├── drizzle.config.ts
...
Let's step through this, first and formost, i'm using `bun` anything should work `pnpm` for example, but for this i'm going to use `bun`. The first thing you should do is run `bun init`, and bootstrap your typescript project. Now you can validate the `tsconfig.json` and `tsup.config.ts` files. For this simple example you can use these.
